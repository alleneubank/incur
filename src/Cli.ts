  ...
  if (skillsIdx !== -1) {
    ...
    try {
      ...
      const result = await SyncSkills.sync(name, commands, {
        contextRules: sync?.contextRules,
        cwd: sync?.cwd,
        depth,
        description: options.description,
        global,
    ...
    if (
      ...
      writeln(
        Help.formatCommand(name, {
          ...
          hint: cmd.hint,
          options: getCommandOptionsSchema(cmd),
      ...
      if (isRoot && options.rootCommand && helpCmds.size > 0) {
        ...
        writeln(
          Help.formatRoot(helpName, {
            ...
            env: isRoot ? options.envSchema : undefined,
            envSource: options.env,
            globals: options.globals,
...
function formatHumanValidationError(
  ...
  lines.push(
    Help.formatCommand(path === cli ? cli : `${cli} ${path}`, {
      ...
      envSource,
      hint: command.hint,
      options: getCommandOptionsSchema(command),
...
function commandManifestEntry(
  ...
  const bodySchema = entry.body ? Schema.toJsonSchema(entry.body) : undefined
  const fullInputSchema = entry.input ? Schema.toJsonSchema(entry.input) : undefined
