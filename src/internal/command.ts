...
export async function execute(command: any, options: execute.Options): Promise<execute.Result> {
  ...
  const runCommand = async () => {
    ...
    if (parseMode === 'argv') {
      ...
      args = parsed.args
      parsedOptions = effectiveOptions ? Parser.zodParse(effectiveOptions, inputOptions) : {}
    } else {
      ...
      parsedOptions = effectiveOptions ? Parser.zodParse(effectiveOptions, split.options) : {}
    }
