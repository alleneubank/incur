...
export async function sync(
  ...
  try {
    ...
    for (const file of files) {
      ...
      // Report the same normalized name the file was staged under, so the
      // install result and the frontmatter never disagree.
      ...
      for (const pattern of options.include) {
        ...
        for await (const match of fs.glob(globPattern, { cwd })) {
          ...
          try {
            ...
            // `parseSkillFrontmatter` so YAML-quoted descriptions (written by
